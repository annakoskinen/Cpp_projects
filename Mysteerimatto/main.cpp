...
// Funktio kysyy käyttäjältä maton leveyden ja korkeuden ja kutsuu funktiota split,
// joka jakaa syötetyt arvot leveydeksi ja korkeudeksi.
// Funktio palauttaa vektorin, joka sisältää leveyden ja korkeuden arvot.
...
// Funktio muuttaa syötteenä annetun merkkijonon merkit Color tyyppisiksi arvoiksi ja
// samalla tarkistaa, ettei merkkijono sisällä värejä, joita ei löydy luettelotyypistä
// Color. Parametreinä syötetty merkkijon ja vektori, johon Color tyyppiset arvot
// tallennetaan.
// Funktio palauttaa totuusarvon false, jos merkkiä ei ole oikea väri. Totuusarvo true
// palautetaan, jos merkkien muuttaminen Color tyyppisiksi onnistuu.
bool inputToColor(std::string color_input, std::vector<Color> &string_to_color)
{
    // Käydään läpi merkkijonon color_input arvot ja muutetaan ne Color tyyppisiksi.
    // Palautetaan false, jos merkki ei ole yksi halutuista väreistä.
...
// Funktio muodostaa maton, jos käyttäjä antaa maton värit syötteenä. Parametreinä
// vektori carpet, johon sijoitetaan muodostettu matto, vektori string_to_color,
// joka sisältää halutun maton arvot Color tyyppisinä sekä kokonaisluku maton leveys.
void generateInputCarpet(std::vector<std::vector<Color>> &carpet,
                         std::vector<Color> string_to_color, int width)
{
...
// Funktio kysyy käyttäjältä, miten hän haluaa muodostaa maton. Kysymystä jatketaan
// kunnes käyttäjä syöttää oikean kirjaimen. Funktio kutsuu syötteestä riippuen
// oikeaa funktiota.
// Parametreinä vektori carpet maton luomista varten, sekä kokonaislukuina maton
// leveys ja korkeus.
void start(std::vector<std::vector<Color>> &carpet, int width, int height)
{
    // While-loop:ia jatketaan niin kauan kuin keep_going totuusarvo on true.
    std::string start_input = "";
    bool keep_going = true;
    while(keep_going) {
        // Kysytään käyttäjää syöttämään R tai I riippuen halutusta maton
        // muodostustavasta.
        ...
        // täyttämistä varten ja tarkistetaan, että syötetään oikea määrä
        // oikeita värejä.
            ...
            // Luodaan vektori, johon voidaan sijoittaa merkkijonon värit
            // Color tyyppeinä.
            std::vector<Color> string_to_color = {};

            // Tarkistetaan, että syötetty merkkijono on tarpeeksi pitkä halutun
            // kokoisen maton luomista varten.
            ...
            // Tarkistetaan funktion inputToColor avulla, että merkkijono sisältää
            // vain oikeita värejä. Jos inputToColor paluuarvona on false,
            // tulostetaan virheilmoitus.
            else if(!inputToColor(color_input, string_to_color)) {
            ...
            // luomista varten ja lopetetaan while-loop.
                ...
                generateInputCarpet(carpet, string_to_color, width);
                keep_going = false;
            }
        }
...
// Funktio tulostaa muodostetun maton. Parametreinä vektori carpet, joka sisältää
// tiedot maton sisällöstä.
void printCarpet(std::vector<std::vector<Color>> const &carpet)
{
    // Käydään carpet-vektorin sisällä olevan row-vektorin sisältämät arvot läpi
    // ja muutetaan Color tyyppiset arvot ENUM_TO_COLOR:in avulla oikeaa väriä
    // vastaavaksi kirjaimeksi. Tulostetaan värit riveittäin.
                 ...
                 std::vector<std::vector<Color>> carpet)
{
    ...
    // Tulostetaan myös osumien määrä ja tyhjennetään koordinaatit ja kysytyn
    // kuvion vektori.
...
// Parametrinä vektori carpet, jossa on aiemmin muodostettu matto.
// Funktio palauttaa totuusarvon false, jos käyttäjää syöttää kirjaimen q.
// True palautetaan, jos mallin syöttäminen onnistuu.
bool inputPatterns(std::vector<std::vector<Color>> carpet)
{
    std::string pattern = "";
    bool keep_going = true;
    ...
    while(keep_going) {
        ...
        // lisätään ne merkkijonoon
        //pattern.
        for(int i = 0; i < pattern_length; ++i) {
            char upper_case_letter = toupper(pattern.at(i));
            pattern += upper_case_letter;
        }

        // Poistetaan merkkijonosta pattern alkuperäinen syötetty osa, jolloin jäljelle
        // jää vain isoja kirjaimia sisältävä värimalli.
        pattern.erase(0, pattern_length);
        ...
        // Tarkistetaan, että syötetty malli on oikean pituinen. Jos pituus on eri kuin
        // neljä, tulostetaan virheilmoitus.
        else if(pattern_length != 4) {
            std::cout << " Error: Wrong amount of colors." << std::endl;
        }

        // Tarkistetaan funktion inputToColor avulla, että merkkijono sisältää vain
        // oikeita värejä. Jos inputToColor paluuarvona on false,
        // tulostetaan virheilmoitus.
        else if(! inputToColor(pattern, pattern_to_color)) {
            std::cout << " Error: Unknown color." << std::endl;
        }

        // Jos syötetty malli on oikeanlainen, kutsutaan funktiota findPattern,
        // jolla mallia etsitään matosta.
        else {
...
int main()
{
    // Muodostetaan vektori numerics, joka sisältää tiedot maton leveydestä
    // ja korkeudesta.
    std::vector<std::string> numerics = firstInput();

    // Vektorin numerics ensimmäinen arvo on leveys ja toinen korkeus.
    // Muutetaan nämä arvot string tyyppisistä kokonaisluvuiksi.
    int width = std::stoi(numerics.at(0));
    int height = std::stoi(numerics.at(1));

    // Tarkistetaan, että matto on oikean kokoinen. Jos leveys tai korkeus on
    // pienempi kuin 2, tulostetaan virheilmoitus ja lopetetaan ohjelma
    // paluuarvolla EXIT_FAILURE.
    ...
    // Kutsutaan funktiota start, jotta saadaan selville, miten matto halutaan
    // muodostaa.
    ...
    // Kutsutaan funktiota, joka kysyy värimalleja. Jos funktion paluuarvo on false,
    // ohjelman suoritus loppuu paluuarvoon EXIT_SUCCESS.
    if(! inputPatterns(carpet)) {
        return EXIT_SUCCESS;
    }
}
